...
impl partial_model {
    pub fn new(before: usize, after: usize) -> Self { Self { before, after } }
}
...
=======
    ...
    let estrides = strides.unwrap_or(1);
    // restrict time/data to the viz slice 
    let tall: Vec<f64> = tfull.slice(s![eoffset..(eoffset+elimit); estrides] ).to_vec();
    let dall: Vec<f64> = dfull.slice(s![eoffset..(eoffset+elimit); estrides] ).to_vec();

    if ( estrides > 1 ){
        panic!( " estrides Not yet supported ");
    }
    // model (time) clipped to viz slice 
    let ETS : Vec<(usize, &f64)> = tsplice.iter().enumerate().filter(|(k,g)|{ ( (**g as usize) >= eoffset ) && ( ( **g as usize) < ( eoffset + elimit ))}).collect();
    let etsplice : Vec<f64> = ETS.iter().map(|(a,b)|{**b}).collect();
    let etslice : Vec<usize> = ETS.iter().map( |(a,b)|{ *a }).collect();
    
    (0..min(top, VV.len())).into_par_iter()
        .for_each(
        |k|
        { 
            //step 4 clipping plane for model
            ...
            let mx = eval_M( &fitted_model , &etsplice);
            //(* flanking prediction *)
            let partial_model{ before: pre, after: post } = pmod;

            let tpre : Vec<f64> = tall.iter().filter( |g| { **g < *etsplice.first().unwrap() } ).map( |v| {*v}).collect();
            let tpre : Vec<f64> = tpre.iter().rev().take( pre ).rev().map( |v| {*v}).collect();//tpre[ 0.max(tpre.len() - pre).. ].to_vec();
            let tpost : Vec<f64> = tall.iter().filter( |g | { **g > *etsplice.last().unwrap() }).map( |v| {*v} ).take( post ).collect();
            ...
            viz_lib2::plot_c( 
                ...
                &tpost, 
                &mxpost, 
                viz_lib2::PlotAction::PNG(format!( "{}_fit_{}", output_stem.to_str().unwrap(), k).into(), 800, 600, 2.0),
                String::from( format!( "model fit {}.{} LSM/data-point {:.5}", models.as_str().replace(".yml", "") , k, residual_per_point )), 
                String::from( "time "), 
...
pub fn vec_envelope_to_region( vhi: &Vec<f64>, c: &Vec<f64> , vlo: &Vec<f64> ) -> Vec<f64>{
    let mut val_there_and_back :Vec<f64> = vhi.iter()
        .enumerate().map(|(j,x)| { c[j].min( c[j] + *x)}).collect(); 
    let mut data_back = vlo.clone()
                        .iter().enumerate().rev().map( |(j, x)| {c[j].max( c[j] + *x)}).collect();
    val_there_and_back.append( &mut data_back );
    val_there_and_back
}

pub fn residual_visualization(  models: String, 
                                output : String , 
                                item : usize,
                                offset :Option<i64>,  
                                limit : Option<usize>, 
                                ...
                                pmod : partial_model )
    {
        ...
        (item..(item + 1 )).into_par_iter().for_each(
            |k|
            { 
                let model_fit{ humps : humps,fitted_model : fitted_model, initial_model : initial_model, residual_total : residual_total,residual_per_point : residual_per_point} = &VV[k] ;
                let mx = eval_M( &fitted_model , &etsplice);
                ...
                let ipre : Vec<usize> = tall.iter().enumerate().filter(|(k,g)| { **g < *etsplice.first().unwrap() }).map( |(k,v)|{ k } ).collect();
                let ipre : Vec<usize> = ipre.iter().rev().take( pre).rev().map( |k| {*k}).collect(); 
                let tpre : Vec<f64> = tall.iter().filter( |g| { **g < *etsplice.first().unwrap() } ).map( |v| {*v}).collect();
                let tpre : Vec<f64> = tpre.iter().rev().take( pre ).rev().map( |v| {*v}).collect();//tpre[ 0.max(tpre.len() - pre).. ].to_vec();
                let tpost : Vec<f64> = tall.iter().filter( |g | { **g > *etsplice.last().unwrap() }).map( |v| {*v} ).take( post ).collect();
                let ipost : Vec<usize> = tall.iter().enumerate().filter( |(k,g)| { **g > *etsplice.last().unwrap() }).map( |(k,g)| {k }).take(post).collect();
                ...
                let i_viz : Vec<usize> = ipre.iter().chain( etslice.iter().chain( ipost.iter() )).map( |k| {*k}).collect();
                //let res_pre : Vec<f64> = ipre.iter().map(|k|{resid1xx[*k]}).collect();
                //let res_mod : Vec<f64> = etslice.iter().map( |k | { resid1xx[*k] }).collect();
                //let res_post : Vec<f64> = ipost.iter().map(|k|{resid1xx[*k]}).collect();
                ///////////////
                ...
                let regions_all : Vec<Vec<f64>> = (0..(qpn/2)).map(
                    |k| 
                    { 
                        let data_vizslice_there_and_back: Vec<f64> = vec_envelope_to_region( &quantiles_running[k] , &m_viz, &quantiles_running[ qpn - (k+1) ]  );
                        data_vizslice_there_and_back
                    }
                ).collect();
...
pub fn intermediate_visualization(models: String, output: String, item: usize, pval : f64, 
